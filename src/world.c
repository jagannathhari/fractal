    ...
    while ((line = read_line(f)))
    {
        ...
        if (STR_EQL("initial_state", command))
        {
            if (sscanf(line,
                       ...
                       &world->x,
                       &world->y,
                       &world->initital_angle) != 3
            )
                goto error;
        }
        else if (STR_EQL("length", command))
        {
            if (sscanf(line, "length %f", &world->length) != 1)
                goto error;
        }
        else if (STR_EQL("angle", command))
        {
            if (sscanf(line, "angle %f", &world->angle) != 1)
                goto error;
        }
        else if (STR_EQL("name", command))
        {
            if (sscanf(line, "name %254s", world->world_name) != 1)
                goto error;
        }
        else if (STR_EQL("levels", command))
        {
            if (sscanf(line, "levels %d", &world->level) != 1)
                goto error;
        }
        else if (STR_EQL("rules", command))
        {
            ...
            while ((productions = read_line(f)))
            {
                ...
                if (vertex)
                {
                    ...
                    memcpy(temp, &productions[2],
                           vector_length(productions) - 2);
